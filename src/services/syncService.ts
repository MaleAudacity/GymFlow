...
export const backupToSupabase = async (): Promise<SyncResult> => {
  try {
    ...
    if (!user) {
      return {
        success: false,
        message: 'You must be logged in to sync with Supabase cloud.',
      };
    }
    ...
    // 2. Sync Plans (Upsert with user_id)
    if (plans.length > 0) {
    ...
    // 3. Sync Members (Upsert with user_id)
    if (members.length > 0) {
    ...
    return {
      success: false,
      message: err?.message || 'Failed to backup data to Supabase.',
      error: err?.message,
    };
  }
};
 ...
 * Used on new phone or after app re-installation
 */
export const restoreFromSupabase = async (): Promise<SyncResult> => {
  try {
    ...
    return {
      success: false,
      message: err?.message || 'Failed to restore data from Supabase.',
      error: err?.message,
    };
  }
};
